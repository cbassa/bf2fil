...
/* Compute the spectral kurtosis mask. Input is the dynamic spectrum z
   ...
   the dynamic spectrum will be replaced by the average of that spectrum.
 */
...
{
  int64_t i,j,k,l;
  int mact,nmask=0,flag;
  double s1,s2,s3,sk;
  float zavg,zstd,dz;
  float *zbm,*zbs;
  float *cs1,*cs2,*bs1,*bs2;

  // Block mean and standard deviation
  zbm=(float *) malloc(sizeof(int)*nx*my);
  zbs=(float *) malloc(sizeof(int)*nx*my);

  // Channel sums
  cs1=(float *) malloc(sizeof(int)*nx);
  cs2=(float *) malloc(sizeof(int)*nx);

  // Block sums
  bs1=(float *) malloc(sizeof(int)*nx*my);
  bs2=(float *) malloc(sizeof(int)*nx*my);
  ...
  for (i=0;i<nx;i++) {
	...
	nmask++;
      ...
      // Compute mean and standard deviation of block
      zbm[l]=s1/(float) m;
      zbs[l]=s2/(float) m-zbm[l]*zbm[l];
      zbs[l]=sqrt(zbs[l]);
      bs1[l]=s1;
      bs2[l]=s2;
  ...
  // Deselect on standard deviation
  for (j=0;j<my;j++) {
    // Average                                                                                   
    s1=0.0;
    s2=0.0;
    ...
    // Recompute stats
    for (l=0,flag=0;l<20;l++) {
      for (i=0;i<nx;i++) {
	// skip masked samples
	if (mask[i+nx*j]!=0)
	  continue;
	if (fabs(zbs[i+nx*j]-zavg)>sigma*zstd) {
	  mask[i+nx*j]=2;
	  nmask++;
	  s1-=zbs[i+nx*j];
	  dz=zbs[i+nx*j]-zavg;
  ...
  for (j=0;j<my;j++) {
	...
	if (mask[i+nx*j]!=0)
	  continue;
	if (fabs(zbm[i+nx*j]-zavg)>sigma*zstd) {
	  mask[i+nx*j]=3;
	  nmask++;
	  s1-=zbm[i+nx*j];
	  dz=zbm[i+nx*j]-zavg;
  ...
  for (i=0;i<nx;i++) {
    ...
    for (j=0;j<my;j++) {
      l=i+nx*j;
      // Decrease sums for masked blocks
      if (mask[l]!=0) {
	cs1[i]-=bs1[l];
	cs2[i]-=bs2[l];
	nzc[i]-=m;
    ...
    zcm[i]=cs1[i]/nzc[i];
    zcs[i]=cs2[i]/(float) nzc[i]-zcm[i]*zcm[i];
    zcs[i]=sqrt(zcs[i]);
  }
